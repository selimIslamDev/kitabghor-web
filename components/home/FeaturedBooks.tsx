...
function BookCard({ book, onAddToCart }: { book: Book; onAddToCart: (e: React.MouseEvent<HTMLButtonElement>, book: Book) => void }) {
  ...
  return (
    <Link
      ...
      className="relative bg-white dark:bg-slate-800 rounded-3xl border border-[var(--border)] p-5 hover:shadow-xl transition-all duration-200 hover:-translate-y-1 group block cursor-pointer"
    >
      {/* Book cover - fixed size, centered like a real book */}
      <div className="relative mx-auto mb-4" style={{ width: "180px", height: "230px" }}>
        <div className="relative w-full h-full bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-700 dark:to-slate-600 rounded-xl overflow-hidden shadow-md">
        ...
        {/* Scalloped discount seal - overlapping cover top-right corner */}
        {discountPercent && (
          <div className="absolute -top-3 -right-3 z-10 w-16 h-16">
            <div className="absolute inset-0 bg-red-500 rounded-lg shadow-lg" />
      ...
      {/* Info */}
      <h3 className="font-extrabold text-lg uppercase tracking-wide text-gray-900 dark:text-white leading-tight line-clamp-2 mb-1 text-center">
        {book.name}
      </h3>
      <p className="text-sm text-gray-400 dark:text-gray-500 mb-2 text-center">{book.author || "Unknown"}</p>
      ...
      <div className="flex items-center rounded-full overflow-hidden">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 font-bold text-sm tracking-wide transition cursor-pointer ${
            book.stock === 0
        ...
        <button
          ...
          className="flex items-center gap-1 px-4 py-2.5 bg-emerald-100 hover:bg-emerald-200 dark:bg-emerald-900/30 dark:hover:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 font-bold text-xs tracking-wide transition cursor-pointer"
        >
          <Heart className={`w-4 h-4 transition ${wishlisted ? "fill-emerald-600 text-emerald-600" : ""}`} />
          WISHLIST
        </button>
      </div>
    </Link>
    ...
    <section className="py-16 bg-[var(--card)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="bg-white dark:bg-slate-800 rounded-3xl border border-[var(--border)] overflow-hidden animate-pulse p-5">
                <div className="mx-auto rounded-xl bg-[var(--muted)] mb-4" style={{ width: "180px", height: "230px" }} />
                <div className="h-3 bg-[var(--muted)] rounded w-1/3 mb-2 mx-auto" />
                <div className="h-4 bg-[var(--muted)] rounded w-3/4 mb-3 mx-auto" />
        ...
        {!isLoading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {books.length > 0 ? (
              books.map((book) => <BookCard key={book.id} book={book} onAddToCart={handleAddToCart} />)
            ) : (
