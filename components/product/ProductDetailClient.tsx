...
// Helper to check valid meta fields
...
export default function ProductDetailClient({ id }: { id: string }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <Link href="/products" className="inline-flex items-center gap-2 text-sm text-violet-600 dark:text-violet-400 font-medium hover:gap-3 transition-all mb-6">
        <ArrowLeft className="w-4 h-4" />
        Back to Shop
      </Link>

      {/* Main Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 mb-16 relative">
        
        {/* Left: Image Column (5 Cols) */}
        <div className="lg:col-span-5">
          <div
            className="bg-slate-50 dark:bg-slate-900/40 rounded-2xl h-[420px] sm:h-[450px] flex items-center justify-center relative overflow-hidden border border-slate-200/80 dark:border-slate-800 shadow-sm select-none cursor-crosshair p-6"
          ...
          >
            {mainImage && mainImage.startsWith("http") ? (
              <img
                src={mainImage}
                alt={product.name}
                className="max-h-full max-w-full object-contain rounded-md shadow-[0_12px_28px_-6px_rgba(0,0,0,0.18)] transition-transform duration-300 hover:scale-105"
              />
            ) : (
            ...
            {product.discountPrice && (
              <div className="absolute top-4 left-4 bg-rose-500 text-white text-xs font-bold px-3 py-1.5 rounded-full z-10 shadow-sm">
                {discountPercent}% OFF
              </div>
            )}

            {product.stock === 0 && (
              <div className="absolute inset-0 z-20 bg-white/70 dark:bg-slate-900/70 backdrop-blur-[1px] flex items-center justify-center">
            ...
            <div className="flex gap-3 mt-4 justify-center sm:justify-start">
              {product.images.map((img: string, i: number) => (
                <button
                  ...
                  className={`w-14 h-18 rounded-lg overflow-hidden border-2 transition cursor-pointer bg-white dark:bg-slate-900 p-1 ${
          ...
          {/* Trust Badges */}
          <div className="hidden lg:grid grid-cols-2 gap-3 mt-6 p-4 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-200/80 dark:border-slate-800">
            <div className="flex items-center gap-2.5 text-xs font-medium text-slate-600 dark:text-slate-300">
              <div className="w-7 h-7 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center shrink-0">
                <Truck className="w-3.5 h-3.5 text-violet-600 dark:text-violet-400" />
              </div>
              Fast Delivery
            </div>
            <div className="flex items-center gap-2.5 text-xs font-medium text-slate-600 dark:text-slate-300">
            ...
            <div className="flex items-center gap-2.5 text-xs font-medium text-slate-600 dark:text-slate-300">
              <div className="w-7 h-7 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center shrink-0">
                <BadgeCheck className="w-3.5 h-3.5 text-violet-600 dark:text-violet-400" />
              </div>
              100% Original
            </div>
            <div className="flex items-center gap-2.5 text-xs font-medium text-slate-600 dark:text-slate-300">
        ...
        <div className="lg:col-span-7 relative">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              {product.classLevel && (
                <span className="bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-300 text-xs font-medium px-2.5 py-0.5 rounded-md border border-blue-100 dark:border-blue-900/30">
                  {product.classLevel}
                </span>
              )}
              {product.subject && (
                <span className="bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-300 text-xs font-medium px-2.5 py-0.5 rounded-md border border-emerald-100 dark:border-emerald-900/30">
                  {product.subject}
                </span>
              )}
              {product.productType && (
                <span className="bg-violet-50 dark:bg-violet-950/40 text-violet-600 dark:text-violet-300 text-xs font-medium px-2.5 py-0.5 rounded-md border border-violet-100 dark:border-violet-900/30">
            ...
            {/* Title */}
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2 leading-snug">
              {product.name}
            </h1>

            {product.author && isValidMeta(product.author) && (
              <p className="text-gray-500 dark:text-gray-400 text-sm mb-4">
                by <span className="font-semibold text-violet-600 dark:text-violet-400">{product.author}</span>
              </p>
            )}
            ...
            {/* Price Box */}
            <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200/80 dark:border-slate-800 my-4">
            ...
            {/* Metadata Table (Only rendered if valid) */}
            {hasValidMeta && (
              <div className="text-sm my-4 rounded-xl border border-slate-200/80 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800 overflow-hidden">
                {isValidMeta(product.publisher) && (
                  <div className="flex justify-between px-4 py-2.5 bg-white dark:bg-slate-900">
                    ...
                    <span className="text-gray-800 dark:text-gray-200 font-medium text-xs">{product.publisher}</span>
                  </div>
                )}
                {isValidMeta(product.edition) && (
                  <div className="flex justify-between px-4 py-2.5 bg-white dark:bg-slate-900">
                    <span className="text-gray-500 text-xs">Edition</span>
                    <span className="text-gray-800 dark:text-gray-200 font-medium text-xs">{product.edition}</span>
                  </div>
                )}
                {isValidMeta(product.isbn) && (
                  <div className="flex justify-between px-4 py-2.5 bg-white dark:bg-slate-900">
                    <span className="text-gray-500 text-xs">ISBN</span>
                    <span className="text-gray-800 dark:text-gray-200 font-medium text-xs font-mono">{product.isbn}</span>
                  </div>
                )}
                {isValidMeta(product.brand) && (
                  <div className="flex justify-between px-4 py-2.5 bg-white dark:bg-slate-900">
                    <span className="text-gray-500 text-xs">Brand</span>
                    <span className="text-gray-800 dark:text-gray-200 font-medium text-xs">{product.brand}</span>
                  </div>
                )}
                {isValidMeta(product.model) && (
                  <div className="flex justify-between px-4 py-2.5 bg-white dark:bg-slate-900">
                    <span className="text-gray-500 text-xs">Model</span>
                    <span className="text-gray-800 dark:text-gray-200 font-medium text-xs">{product.model}</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="flex items-center gap-3 mt-6 flex-wrap">
              <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 rounded-full px-2 py-1 border border-slate-200 dark:border-slate-700">
                <button
                  onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white dark:hover:bg-slate-700 transition text-gray-600 dark:text-gray-300"
                >
                ...
                <button
                  ...
                  className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white dark:hover:bg-slate-700 transition disabled:opacity-40 text-gray-600 dark:text-gray-300"
                >
              ...
              <button
                ...
                className="flex-1 min-w-[180px] flex items-center justify-center gap-2 py-3 bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white rounded-full font-semibold text-sm transition shadow-sm hover:shadow-md active:scale-[0.98]"
              >
              ...
              <button
                ...
                className={`p-3 rounded-full border transition ${wishlisted
                  ? "border-violet-500 bg-violet-50 dark:bg-violet-900/20 text-violet-600"
                  : "border-slate-200 dark:border-slate-700 hover:border-violet-300 text-gray-400 hover:text-violet-500"
                }`}
              >
          ...
          <div className="space-y-4">
            ...
            {reviews.length === 0 ? (
              ...
              reviews.map((review: Review) => (
                <div key={review.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-0.5">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-3 h-3 ${i < review.rating ? "fill-amber-400 text-amber-400" : "text-gray-200 dark:text-slate-700"}`}
                        />
                      ))}
                    </div>
                  </div>
